...
# MODIFICATIONS BY DUNCAN ROBERTS FOR MPHIL PROJECT
# START OF CHANGES
s/ `\([^`]\)/ ` \1/g
s/ ``\([^`]\)/ `` \1/g

# 1. move elipses and abbreviations 'to one side' temporarily, so we can do simple tokenisation of what's left (i.e. full stops)
s=\.\.\.=:::=g
s=e\.g\.=e:g:=g
s=i\.e\.=i:e:=g
# 2. replace full stops
s=\.= . =g
# 3. restore elipses and abbreviations
s=\:::=...=g
s=e:g:=e.g.=g
s=i:e:=i.e.=g
# Commenting out extant full stop tokenisation as it's made redundant by the above
#s=\([^.]\)\([.]\)\([])}>"']*\)[ 	]*$=\1 \2\3 =g
