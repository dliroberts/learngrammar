/*
 * PhraseFactory.java - A class with convenience methods for creating phrases.
 * 
...
public class NLGFactory {
	...
	public VPPhraseSpec createVerbPhrase(Object verb) {
		...
		verbElement = extractVerb(verb, phraseElement);
		setPhraseHead(phraseElement, verbElement);
		return phraseElement;
	}

	/** Extract head verb and particle (if necessary) from a verb string
	 * @param verb
	 * @param phraseElement
	 * @return
	 */
	private NLGElement extractVerb(Object verb, PhraseElement phraseElement) {
		NLGElement verbElement;
		// verb needs special processsing, eg if it has a particle  (eg, "pick up")
		if (verb instanceof String) {
			int space = ((String) verb).indexOf(' ');
			if (space == -1) {
				verbElement = createWord(verb, LexicalCategory.VERB);
			} else {
				verbElement = createWord(((String) verb)
						.substring(0, space), LexicalCategory.VERB);
				phraseElement.setFeature(Feature.PARTICLE, ((String) verb)
						.substring(space + 1, ((String) verb).length()));
			}
		} else {
			verbElement = createNLGElement(verb,LexicalCategory.VERB);
		}
		return verbElement;
	}
	...
	public SPhraseSpec createClause(Object subject, Object verb,
		...
		if (verb != null)
			phraseElement.setVerb(extractVerb(verb, phraseElement));
